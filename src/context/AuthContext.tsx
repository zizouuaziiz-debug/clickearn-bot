...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const [token, setToken] = useState<string | null>(() => {
    ...
    return localStorage.getItem("earnora_token");
  });
  ...
  useEffect(() => {
    ...
    const storedToken = localStorage.getItem("earnora_token");
    if (storedToken) {
  ...
  async function authenticateWithTelegram(initData?: string) {
    ...
    try {
      ...
      localStorage.setItem("earnora_token", data.token);
      setToken(data.token);
      setUser(data.user);
      setAuthError("");
    } catch (error: any) {
  ...
  async function refreshUser(explicitToken?: string): Promise<boolean> {
    try {
      ...
      const storedToken = explicitToken ?? localStorage.getItem("earnora_token");
  ...
  function logout() {
    if (typeof window !== "undefined") {
      localStorage.removeItem("earnora_token");
    }
    setToken(null);
    setUser(null);
    setAuthError("");
