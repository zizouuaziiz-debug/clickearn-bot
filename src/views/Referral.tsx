    ...
    <DashboardLayout>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl p-5 shadow-sm mb-6">
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          <input readOnly value={info?.referralLink ?? ""} className="flex-1 px-3 py-2 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg text-sm text-gray-600 dark:text-gray-300 break-all" />
          <button onClick={copy} className="flex items-center justify-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700 transition-colors sm:w-auto">
        ...
        <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <button
            ...
            className="inline-flex items-center justify-center gap-2 px-4 py-2 bg-sky-500 text-white rounded-lg text-sm hover:bg-sky-600 transition-colors"
          >
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-5 shadow-sm">
          ...
          {referred.length === 0 ? (
            ...
            <div className="space-y-3">
              {referred.map(u => (
                <div key={u.id} className="flex flex-col gap-2 border-b border-gray-100 py-2 text-sm dark:border-gray-700 last:border-0 sm:flex-row sm:items-center sm:justify-between">
          ...
          {(info?.recentCommissions?.length ?? 0) === 0 ? (
            ...
            <div className="space-y-3">
              {info.recentCommissions.map((item: any) => (
                <div key={item.id} className="flex flex-col gap-2 border-b border-gray-100 py-2 text-sm dark:border-gray-700 last:border-0 sm:flex-row sm:items-center sm:justify-between">
                  <div className="min-w-0">
                    <div className="font-medium text-gray-900 dark:text-white break-words">{item.description}</div>
