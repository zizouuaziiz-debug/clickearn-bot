...
export default function Lootably() {
  const { token } = useAuth();
  const { t } = useLang();
  ...
  useEffect(() => {
    if (!token) return;
    fetch("/api/lootably/config", { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.json())
  ...
  return (
    <DashboardLayout>
      <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-5">Lootably Offerwall</h1>

      {loading && <p className="text-gray-500 dark:text-gray-400">{t("loading")}</p>}

      {error && !loading && (
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-5">
          <p className="font-semibold text-amber-800 dark:text-amber-200">Lootably is not available right now.</p>
          ...
          {view === "info" && (
            <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm space-y-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="font-semibold text-gray-900 dark:text-white">Lootably Offers</h2>
                  <p className="text-sm text-gray-500">Complete offers and earn real rewards instantly.</p>
                </div>
              </div>
              <p className="text-sm text-gray-600 dark:text-gray-300">
                Browse hundreds of available offers — install apps, complete surveys, sign up for services, and more.
                Each completed offer earns real cash added to your wallet.
              </p>
