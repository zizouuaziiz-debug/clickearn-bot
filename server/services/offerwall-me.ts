/**
 * Offerwall.me integration service.
 *
 * Generic offerwall integration template.
 * When official docs specify different parameter names, adjust the
 * parsePostbackPayload function below.
 *
 * Env vars:
 * - OFFERWALL_ME_ENABLED
 * - OFFERWALL_ME_PUBLISHER_ID
 * - OFFERWALL_ME_API_KEY
 * - OFFERWALL_ME_SECRET_KEY
 * - OFFERWALL_ME_REWARD_MULTIPLIER (default 1)
 */

import crypto from "crypto";

export interface OfferwallMeConfig {
  enabled: boolean;
  publisherId: string;
  apiKey: string;
  secretKey: string;
...
export function getOfferwallMeConfig(): OfferwallMeConfig {
  return {
    enabled: process.env.OFFERWALL_ME_ENABLED === "true",
    publisherId: process.env.OFFERWALL_ME_PUBLISHER_ID || "",
    apiKey: process.env.OFFERWALL_ME_API_KEY || "",
    secretKey: process.env.OFFERWALL_ME_SECRET_KEY || "",
    rewardMultiplier: parseFloat(process.env.OFFERWALL_ME_REWARD_MULTIPLIER || "1") || 1,
  };
}

export function buildOfferwallUrl(publisherId: string, apiKey: string, userId: string | number) {
  return `https://offerwall.me/wall?pub=${encodeURIComponent(publisherId)}&api_key=${encodeURIComponent(apiKey)}&user_id=${encodeURIComponent(String(userId))}`;
}
...
export interface OfferwallMePostbackPayload {
  userId: string;
...
export function parsePostbackPayload(query: Record<string, unknown>): OfferwallMePostbackPayload | null {
