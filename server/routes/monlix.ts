 ...
 * Monlix routes.
 *
 * - GET  /api/monlix/config      -> offerwall URL
 * - GET  /api/monlix/postback    -> S2S conversion callback
 */
...
import {
  ...
  verifySignature,
} from "../services/monlix";

const router = Router();

router.get("/config", requireAuth, async (req, res) => {
  const config = getMonlixConfig();
  if (!config.enabled) {
    res.status(503).json({ error: "Monlix is not enabled" });
    return;
  }
  if (!config.publisherId || !config.apiKey) {
    res.status(500).json({ error: "Monlix publisher ID or API key is not configured" });
    return;
  }
  ...
  res.json({
    ...
    postbackUrl: `${req.headers["x-forwarded-proto"] || "http"}://${req.headers.host}/api/monlix/postback`,
  });
});
